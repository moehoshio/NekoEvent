 ...
 * using C++20 module imports instead of traditional header includes.
 * It covers event publishing/subscribing, filters, priorities, task scheduling,
...
// Test fixture
class EventLoopModuleTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(EventLoopModuleTest, BasicEventPublishSubscribe) {
...
TEST_F(EventLoopModuleTest, MultipleSubscribers) {
...
TEST_F(EventLoopModuleTest, EventUnsubscribe) {
    std::atomic<int> eventCount{0};
...
TEST_F(EventLoopModuleTest, EventFiltering) {
...
TEST_F(EventLoopModuleTest, EventPriority) {
...
TEST_F(EventLoopModuleTest, BasicTaskScheduling) {
...
TEST_F(EventLoopModuleTest, TaskCancellation) {
    std::atomic<bool> taskExecuted{false};
...
TEST_F(EventLoopModuleTest, RepeatingTask) {
...
TEST_F(EventLoopModuleTest, DelayedEventPublishing) {
...
// Statistics tests
TEST_F(EventLoopModuleTest, EventStatistics) {
...
TEST_F(EventLoopModuleTest, ExceptionHandling) {
 ...
 * Module Test Summary:
 * 
 * This test file verifies that the NekoEvent system works correctly when
 * used via C++20 module imports instead of traditional header includes.
 * 
 * All tests have been designed to handle race conditions by:
 * - Adding startup delay after launching event loop thread
 ...
 *  DelayedEventPublishing - Tests delayed event publishing
